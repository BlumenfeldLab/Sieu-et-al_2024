function Behavior = LickTaskBehavior_ctrl(seizurePaths, seizureCell, params, varargin)
% Created By: Xinyuan Zheng - xinyuan.zheng@outlook.com
% Modified Abdo's LickTaskBehavior() for control periods
%**************************************************************************
...
ClickResponses = zeros(sum(strcmp(toInclude,'Include')), 5*4+1);
...
for file = 1:length(seizurePaths) % file = 112
...
%     disp(file);
    ...
    seizureInfo = getControlTimingInfo(filepath, seizureCell, params);
    if seizureInfo.hasControl
        szCount = szCount + 1; 
    else
        disp('err');
        continue;
    ...
    AllTimes = seizureInfo.AllTimes_ctrl;
